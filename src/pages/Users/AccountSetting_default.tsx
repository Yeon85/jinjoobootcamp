...
const AccountSetting = () => {
    ...
    const handleUpload = async () => {
        ...
        formData.append('userId', String(user.id)); // ✅ userId 추가!

        console.log("String(user.id)",String(user.id));
        try {
            const response = await axios.post(`${API_URL}/api/upload-profile`, formData, {
                headers: {
            ...
            // 이후, 프로필 사진을 users 테이블에 저장하려면 여기서 추가 API 호출
            // 예시: await axios.put('/api/user/profile-image', { id: user.id, profileImage: filePath });
  ...
  alt="Profile"
  className="w-20 h-20 md:w-32 md:h-32 rounded-full object-cover mx-auto mb-3"
/> */}

<img
        src={previewImage || (user.profileImage ? `${API_URL}${user.profileImage}` : '/assets/images/user-profile.jpeg')}
        alt="Profile"
        className="w-20 h-20 md:w-32 md:h-32 rounded-full object-cover mx-auto mb-3"
    />
                                   {!selectedImage ? (
        <label className="btn btn-light w-full">
            파일 선택
            <input
                type="file"
                accept="image/*"
