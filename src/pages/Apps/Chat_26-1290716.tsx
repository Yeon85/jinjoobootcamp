      ...
      <div className={`flex gap-5 relative sm:h-[calc(100vh_-_150px)] h-full sm:min-h-0 ${isShowChatMenu ? 'min-h-[999px]' : ''}`}>
        ...
        <div
          className={`panel p-0 flex-none max-w-xs w-full absolute xl:relative z-10 space-y-4 xl:h-full overflow-hidden
            ${(!isShowUserChat || isShowChatMenu) ? 'block' : 'hidden'} xl:block
          `}
        >
          <div className="p-4">
            {/* header */}
        ...
        {/* RIGHT */}
        <div className={`panel p-0 flex-1 ${!isShowUserChat ? 'hidden xl:block' : 'block'}`}>
          {/* 데스크탑(xl)에서는 선택 전 안내 보여도 됨 */}
          {!isShowUserChat && (
