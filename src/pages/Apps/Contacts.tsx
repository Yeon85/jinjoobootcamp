...
const Contacts = () => {
    ...
    const deleteUser = () => {
        showMessage('삭제는 아직 구현되지 않았습니다.', 'info');
    };
    ...
    const createContact = async () => {
        ...
        if (!params.targetUserId) {
            showMessage('친구 User ID를 입력해주세요.', 'error');
            return;
        }
        if (!params.name) {
            showMessage('친구 이름을 입력해주세요.', 'error');
            return;
        }
        ...
        try {
            ...
            console.error(e);
            const msg = e?.response?.data?.message || e?.response?.data || '친구 추가 실패';
                                ...
                                {filteredItems.map((contact: any) => {
                                    return (
                                        <tr key={contact.userId}>
                                            ...
                                            <td className="whitespace-nowrap">{contact.userId}</td>
                                            <td className="whitespace-nowrap max-w-[320px] truncate">{contact.preview || ''}</td>
                                            <td>
                                                <div className="flex gap-4 items-center justify-center">
                                                    <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => deleteUser()}>
                                                        Delete
                                                    </button>
                                                </div>
                                            </td>
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="p-5">
                                        <form>
                                            <div className="mb-5">
                                                <label htmlFor="targetUserId">Friend User ID</label>
                                                <input
                                                    id="targetUserId"
                                                    type="number"
                                                    placeholder="예) 2"
