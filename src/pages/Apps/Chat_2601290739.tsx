...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const onNewMessage = (msg: Message) => {
      ...
      setContactList((prev) =>
        prev.map((c) => (c.contactId === msg.contactId ? { ...c, messages: [...(c.messages || []), msg], preview: msg.text, time: msg.time } : c))
      );
  ...
  // 검색 필터 (tab 관계없이 contactList에서 이름으로 필터)
  useEffect(() => {
    const keyword = searchUser.toLowerCase();
    setFilteredItems(contactList.filter((d) => (d.name || '').toLowerCase().includes(keyword)));
