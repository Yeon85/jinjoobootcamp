...
const Chat = () => {
  ...
  useEffect(() => {
    setFilteredItems(contactList.filter((c) => (c.name || '').toLowerCase().includes(searchUser.toLowerCase())));
  ...
  return (
    <div className="flex sm:h-[calc(100vh_-_150px)] h-full">
      {/* ================= LEFT ================= */}
      <div className="panel p-4 w-full max-w-none xl:max-w-xs">
        ...
        <div className="relative mb-3">
          <input className="form-input pr-9" placeholder="검색" value={searchUser} onChange={(e) => setSearchUser(e.target.value)} />
        ...
        <div className="flex justify-between text-xs mb-3">
          ...
          <button onClick={() => setTab('chats')} className={tab === 'chats' ? 'text-primary' : ''}>
            <IconMessagesDot />
            채팅
          </button>
          ...
          <button onClick={() => setTab('contacts')} className={tab === 'contacts' ? 'text-primary' : ''}>
            <IconUserPlus />
            연락처
          </button>
          <button onClick={() => setTab('noti')}>
            <IconBell />
            공지
          </button>
        </div>
        ...
        <PerfectScrollbar className="h-[calc(100vh_-_360px)] space-y-2">
            ...
            <>
              ...
              {filteredItems.map((c) => (
                <div key={c.contactId} className="flex justify-between items-center p-2 hover:bg-gray-100 rounded-md">
                  ...
                  <button className="btn btn-sm btn-outline-primary" onClick={() => selectUser(c)}>
                    Chat
                  </button>
                </div>
              ))}
          ...
          {tab === 'chats' &&
            filteredItems.map((c) => (
              <button key={c.contactId} onClick={() => selectUser(c)} className="w-full flex justify-between p-2 hover:bg-gray-100 rounded-md">
      ...
      {/* ================= RIGHT (채팅) ================= */}
      {/* ✅ h-full 추가해서 내부 중앙정렬이 정확히 먹게 함 */}
      <div className="panel flex-1 hidden xl:block h-full">

        {!isShowUserChat ? (
          // ✅ 완전 중앙 정렬 (아이콘 위 / 텍스트 아래)
          <div className="h-full flex flex-col items-center justify-center text-white-dark">
            <IconMessage className="mb-3 text-4xl opacity-60" />
            <p className="text-sm">채팅을 선택하세요</p>
          </div>
        ) : (
          <div className="p-4 h-full">채팅 화면 영역 (기존 로직 그대로 연결)</div>
        )}
      </div>
      ...
      <Transition appear show={addContactModal} as={Fragment}>
        <Dialog as="div" open={addContactModal} onClose={() => setAddContactModal(false)}>
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
            <div className="panel w-[400px] p-4 relative">
              <button className="absolute right-4 top-4" onClick={() => setAddContactModal(false)}>
                <IconX />
              </button>
              ...
              <button className="btn btn-primary w-full" onClick={createContact}>
                추가
              </button>
            </div>
          </div>
