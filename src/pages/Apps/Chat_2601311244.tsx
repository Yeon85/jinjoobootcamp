...
const Chat = () => {
  ...
  const resolveImg = (p?: string) => {
    if (!p) return `/assets/images/user-profile.png`;
  ...
  // ✅ 친구추가 (※ API 경로가 다르면 `${API_URL}/api/friends` 를 너 서버에 맞게 수정)
  const addFriend = async (targetUserId: string) => {

    console.log("targetUserId:", targetUserId); 
    try {
      await axios.post(`${API_URL}/api/friends`, {
   ...
   // ✅ 친구삭제 (※ API 경로가 다르면 `${API_URL}/api/friends` 를 너 서버에 맞게 수정)
 // const addFriend = async (targetUserId: number) => {
  const delFriend = async (targetUserId: string) => {
    try {
      await axios.post(`${API_URL}/api/friends`, {
        fromUserId: user.nameId,
        toUserId: targetUserId,
  ...
  useEffect(() => {
    ...
    // ✅ chats / contacts / users 는 같은 contactList 기반 (임시)
  ...
  const sendMessage = async () => {
    ...
    const payload = {
      contactId,
      fromUserId: loginUser.nameId,
      toUserId: selectedUser.nameId,
      text: textMessage,
    };
        ...
        >
          <div className="p-4 flex flex-col h-full min-h-0">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <div className="mx-3">
                  ...
                  <p className="text-xs text-white-dark">{user.nameId}</p>
                </div>
              </div>
            ...
            <div className="relative">
              <input type="text" className="form-input peer ltr:pr-9 rtl:pl-9" placeholder="검색" value={searchUser} onChange={(e) => setSearchUser(e.target.value)} />
                      ...
                      <div key={`${person.contactId}-${person.nameId}`}>
                        <button
                          ...
                          className={`w-full flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-[#050b14] rounded-md dark:hover:text-primary hover:text-primary ${
                            selectedUser && selectedUser.nameId === person.nameId ? 'bg-gray-100 dark:bg-[#050b14] dark:text-primary text-primary' : ''
                          }`}
                        ...
                        <div className="w-full flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-[#050b14] rounded-md">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="ltr:text-left rtl:text-right">
                              ...
                              <p className="text-xs text-white-dark">ID: {c.nameId}</p>
                            </div>
                          </div>
                            {/* ✅ 친구추가 + 채팅 */}
                            <div className="flex items-center gap-2">
                              <button type="button" className="btn btn-sm btn-outline-success" onClick={() => delFriend(c.nameId)}>
                                친구삭제
                              </button>
                    ...
                    filteredItems.map((c: Contact) => (
                      <div key={`${c.contactId}-${c.nameId}`}>
                        <div className="w-full flex justify-between items-center p-2 hover:bg-gray-100 dark:hover:bg-[#050b14] rounded-md">
                          ...
                          {/* ✅ 친구추가 + 채팅 */}
                          <div className="flex items-center gap-2">
                             <button type="button" className="btn btn-sm btn-outline-success" onClick={() => addFriend(c.nameId)}>
                              친구추가
                            </button> 
                            <button type="button" className="btn btn-sm btn-outline-primary" onClick={() => selectUser(c)}>
                              Chat
                    ...
                    {selectedUser.messages && selectedUser.messages.length ? (
                      <>
                        {selectedUser.messages.map((message: Message, index: number) => {
                          const isMine = message.fromUserId === loginUser.nameId;
                          return (
